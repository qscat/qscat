...
def cast_transects(self):
    ...
    if cast_transects_pre_checks_result:
        ...
        globals()['task'] = QgsTask.fromFunction(
            'Load shoreline task',
            load_shorelines_task,
            on_finished=load_shorelines_completed,
            shorelines_params=shorelines_params
        )
        QgsApplication.taskManager().addTask(globals()['task'])
        #QgsApplication.taskManager().addTask(load_shorelines_task)
        """
        # Getting intersects moved to stats
        # if transects_params['is_clip_transects'] \
        #     or transects_params['is_include_intersections']:
        #     # globals usage:
        #     # https://gis.stackexchange.com/questions/296175/issues-with-qgstask-and-task-manager/325871#325871
        #     globals()['cast_transects_intersects_task'] = GetTransectsIntersectsTask(
        #         transects,
        #         shorelines,
        #         shorelines_params,
        #         transects_params,
        ...
        #     globals()['cast_transects_intersects_task'].taskCompleted.connect(
        #         cast_transects_intersects_task_state_changed)

        #     QgsApplication.taskManager().addTask(globals()['cast_transects_intersects_task'])
        #else:
        # No need to use background tasks (maybe)
        # If both option are not checked then just directly
        # add the transect linestring as layers
        add_transects_layer(
            transects,
            base_angles,
            ...
            self.dockwidget.qmlcb_stats_transects_layer,
            #transects_params["layer_file_location"],
            )
...
def get_orig_baseline_transect_points(baseline, by, n):
    ...
    if by == 0:
        ...
        # TODO: insertVertexAt(), include the last vertex
    elif by == 1:
...
def cast_transect(origin, angle, distance, baseline):
    ...
    """
    # TODO: get other formula to rotate
    ...
    return QgsLineString(
        QgsGeometry(transect_geom).asPolyline()
    )


def get_baseline_angle(
    baseline: QgsLineString,
    distances: float,
    smoothing_val: int
) -> float:
    d_left = distances - (smoothing_val/2)
    d_right = distances + (smoothing_val/2)
    len = baseline.length()
